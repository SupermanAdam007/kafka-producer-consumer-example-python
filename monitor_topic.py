...
def get_topic_statistics(bootstrap_server, topic, consumer_group):
    ...
    consumer = KafkaConsumer(
        ...
        api_version=(3, 7, 0),
        security_protocol="SSL",
        group_id=consumer_group,  # Use the specified consumer group
    )
    ...
    try:
        ...
        print(f"[{dt_str()}]: Total messages in topic: {total_messages}")
        print(
            f"[{dt_str()}]: Total lag for consumer group '{consumer_group}': {total_lag}"
        )
...
def monitor_topic(bootstrap_server, topic, consumer_group, interval):
    """Kafka topic monitoring script."""
    try:
        while True:
            get_topic_statistics(bootstrap_server, topic, consumer_group)
